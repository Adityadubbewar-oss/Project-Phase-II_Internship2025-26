...
def register():
    ...
    if request.method == 'POST':
        ...
        # Use name as username (sanitized)
        ...
        if User.query.filter_by(username=username).first():
            username = username + '_' + email.split('@')[0]

        hashed_password = generate_password_hash(password, method='pbkdf2:sha256')
        new_user = User(username=username, email=email, password=hashed_password)
        db.session.add(new_user)
        db.session.commit()
        flash(f'Account created successfully! Welcome, {name}. Please login.', 'success')
        return redirect(url_for('login'))
...
def feedback():
    ...
    if current_user.email == 'admin@umacd.com':
...
def download_report():
    ...
    if os.path.exists(excel_path):
        return send_file(excel_path,
                         download_name="UMACD Dataset.xlsx",
                         as_attachment=True,
    ...
    return send_file(output,
                     download_name="UMACD_Report.xlsx",
                     as_attachment=True,
                     mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
if __name__ == '__main__':
    with app.app_context():
        ...
        admin_email = 'admin@umacd.com'
        admin_user = User.query.filter_by(username='admin').first()
        if admin_user:
            if admin_user.email != admin_email:
                admin_user.email = admin_email
                db.session.commit()
                print('Admin account email updated to: admin@umacd.com')
        else:
            hashed_pw = generate_password_hash('admin123', method='pbkdf2:sha256')
            admin_user = User(username='admin', email=admin_email, password=hashed_pw)
            db.session.add(admin_user)
            db.session.commit()
            print('Admin account created: admin@umacd.com / admin123')
    app.run(debug=True)
